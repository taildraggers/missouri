...
Daily aggregator of Missouri-located taildragger classified listings,
...
[Mississippi](https://github.com/taildraggers/mississippi) repos' approach,
with the state filter swapped to "MO".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MO` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Missouri"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/missouri/`
...
```html
<iframe
  src="https://taildraggers.github.io/missouri/"
  title="Taildragger Ads in Missouri"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MISSOURI_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
