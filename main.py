"""Aggregates Missouri-located listings from every taildraggers.com
...
ones whose location ends in ", MO".
...
Minnesota/Mississippi repos' approach, with the state filter swapped to "MO".
"""
...
PAGE_TITLE = "Taildragger Ads in Missouri"
STATE_ABBR = "MO"
...
_MISSOURI_LOCATION_RE = re.compile(r",\s*MO\s*$", re.IGNORECASE)


def _is_missouri(location: str) -> bool:
    return bool(_MISSOURI_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        missouri_rows = [row for row in rows if _is_missouri(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(missouri_rows)} in Missouri")
        all_listings.extend(missouri_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Missouri listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Missouri listings: {len(listings)}")
